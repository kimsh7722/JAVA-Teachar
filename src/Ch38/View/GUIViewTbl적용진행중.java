...
public class GUIViewTbl적용진행중 extends JFrame implements ActionListener {
	...
	JLabel lbbookName;
	JLabel lbisLend;
	JTextField txbookCode;
	JTextField txbookName;
	JTextField txisLend;
	// JTextArea area;
	JTable table;
	JScrollPane scroll;
	...
	GUIViewTbl적용진행중() {
		...
		//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		String column[] = { "도서코드", "도서명", "대여가능여부" };
		
		model = new DefaultTableModel(null,column);	//row,column 
		table = new JTable(model);
		
		scroll = new JScrollPane(table);
		//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		scroll.setBounds(10, 230, 465, 215);
		...
		panel.add(btn3);
		panel.add(btn4);
		panel.add(txbookCode);
		panel.add(txbookName);
		panel.add(txisLend);
		panel.add(lbbookCode);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btn1) {
			...
			for (int i = 0; i < list.size(); i++) {
				Object rowdata[] = {list.get(i).getBookCode(), list.get(i).getBookName(),list.get(i).isLend()};
